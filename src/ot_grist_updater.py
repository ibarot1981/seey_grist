...
class OTGristUpdater:
    def __init__(self,
                 ...
                 month_year=None):
        """
        Initialize OTGristUpdater

        :param api_key: Grist API key
        :param doc_id: Grist document ID
        :param ot_table_name: Name of the OT detail table to update
        ...
        self.ot_table_name = ot_table_name or os.getenv('GRIST_DUMP_OT_TABLE_NAME', 'Emp_Dump_OT2')
        ...
        logger.info(f"Targeting OT table: {self.ot_table_name}")
    ...
    def _fetch_table_schema(self):
        ...
        """
        try:
            columns_url = f"{self.base_url}/tables/{self.ot_table_name}/columns"
    ...
    def check_month_year_exists(self):
        ...
        try:
            ...
            url = f"{self.base_url}/tables/{self.ot_table_name}/records"

            logger.info(f"Checking if Month_Year {self.month_year} exists in Grist table '{self.ot_table_name}'")
    ...
    def process_excel_data(self, excel_data):
        """
        Process Excel data and insert only new records based on specified conditions.
        
        :param excel_data: DataFrame containing Excel OT data
        """
        if self.month_year is None:
            logger.error("Month-year is not set. Cannot process OT data.")
            return
        ...
        if self.check_month_year_exists():
            logger.error(f"Error: Records for Month_Year '{self.month_year}' already exist in Grist table '{self.ot_table_name}'. Skipping insertion of all records for this month.")
        ...
        # Filter records based on "Emp Type : Temp / Perm" and "OT Hours"
        initial_rows = len(excel_data)
        
        # Ensure 'Emp_Type' and 'OT_Hours' columns exist before filtering
        if 'Emp_Type' in excel_data.columns and 'OT_Hours' in excel_data.columns:
            # Convert 'OT_Hours' to numeric, coercing errors to NaN
            excel_data['OT_Hours'] = pd.to_numeric(excel_data['OT_Hours'], errors='coerce')
            
            # Filter: Emp_Type is "Perm" AND OT_Hours > 0
            filtered_df = excel_data[
                (excel_data['Emp_Type'] == 'Perm') & 
                (excel_data['OT_Hours'] > 0)
            ]
            
            self._skipped_records_count = initial_rows - len(filtered_df)
            logger.info(f"Filtered out {self._skipped_records_count} records not meeting 'Perm' and 'OT Hours > 0' criteria.")
            excel_data = filtered_df
        else:
            logger.warning("Missing 'Emp_Type' or 'OT_Hours' column for filtering. All records will be considered for insertion.")
            self._skipped_records_count = 0 # No filtering applied, so no records skipped by this logic
        ...
        for _, excel_row in excel_data.iterrows():
            ...
            # Map other columns, ensuring they exist in Grist table schema
            column_mapping = {
                "ESIC_Rate": "ESIC_Rate",
                "OT_Hours": "OT_Hours",
                "OT_Hours_Calc": "OT_Hours_Calc",
                "OT_Rate": "OT_Rate",
                "OT_Amt": "OT_Amt",
                "ESIC_4pct_Amt": "ESIC_4pct_Amt",
                "ESIC_075pct_Amt": "ESIC_075pct_Amt"
            }
            ...
            for excel_col, grist_col in column_mapping.items():
                if excel_col in excel_row and grist_col in self.table_columns:
                    ...
                    if pd.notna(value):
                        ...
                        if excel_col in ["ESIC_Rate", "OT_Hours", "OT_Hours_Calc", "OT_Rate", "OT_Amt", "ESIC_4pct_Amt", "ESIC_075pct_Amt"]:
                            try:
                                grist_fields[grist_col] = float(value)
                            except (ValueError, TypeError):
                                logger.warning(f"Could not convert {excel_col} '{value}' to float for EmpNo {excel_row['SFNo']}. Setting to None.")
            ...
            logger.info(f"Prepared record for insertion: SFNo {excel_row['SFNo']} for {self.month_year}")
        ...
        # Print summary
        self._print_summary([]) # Skipped SFNos are handled by the initial filter count
    ...
    def _insert_records(self, records_to_add):
        ...
        """
        add_url = f"{self.base_url}/tables/{self.ot_table_name}/records"
        logger.info(f"Inserting {len(records_to_add)} new records into {self.ot_table_name}")
    ...
    def _print_summary(self, skipped_sfnos):
        ...
        :param skipped_sfnos: List of SFNos that were skipped (not used in this version, kept for signature consistency)
        """
        logger.info("\n" + "="*50)
        logger.info("OT DUMP UPDATE SUMMARY")
        logger.info("="*50)
        logger.info(f"Month/Year processed: {self.month_year}")
        logger.info(f"Total records from Excel: {self._new_records_count + self._skipped_records_count}")
        logger.info(f"Records inserted into Grist: {self._new_records_count}")
        logger.info(f"Records skipped (due to existing Month_Year or filtering criteria): {self._skipped_records_count}")
        
        logger.info("="*50 + "\n")

    def compare_and_update(self, excel_data):
        """
        Main method to process Excel data
